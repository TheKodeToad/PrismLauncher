...
class GenericProfiler : public BaseProfiler {
    Q_OBJECT
   public:
    GenericProfiler(QObject* parent = 0);

   protected:
    void beginProfilingImpl(LaunchTask* process);
};

GenericProfiler::GenericProfiler(QObject* parent)
    : BaseProfiler(parent)
{}
...
BaseExternalTool* GenericProfilerFactory::createTool(QObject* parent)
{
    return new GenericProfiler(parent);
}
