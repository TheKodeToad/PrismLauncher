...
Application::Application(int& argc, char** argv) : QApplication(argc, argv)
{
    ...
    {
        // FIXME: doesn't have polymc.cfg or multimc.cfg fallback
        m_config = std::make_unique<GlobalConfig>(BuildConfig.LAUNCHER_CONFIGFILE);
        m_config->load();

        // logModel->setMaxLines(getConsoleMaxLines(settings()));
        // logModel->setStopOnOverflow(shouldStopOnConsoleOverflow(settings()));
        // logModel->setOverflowMessage(tr("Cannot display this log since the log length surpassed %1
        // lines.").arg(logModel->getMaxLines()));
    ...
    // Instance icons
    {
        ...
        connect(&config(), &GlobalConfig::updated, this, [this]() { m_icons->directoryChanged(config().iconsDir); });
    ...
    // initialize and load all instances
    {
        ...
        connect(&config(), &GlobalConfig::updated, m_instances.get(), &InstanceList::onConfigUpdate);
    ...
    QString jvmArgs = m_config->jvmArgs;
...
bool Application::createSetupWizard()
{
    bool javaRequired = [this]() {
        ...
        if (currentHostName != oldHostName) {
            updateConfig().lastHostname = currentHostName;
            return true;
        }
        QString currentJavaPath = config().javaPath;
...
GlobalConfig& Application::updateConfig()
{
    m_config->markDirty();
    return *m_config;
}
