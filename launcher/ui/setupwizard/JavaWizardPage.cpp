...
bool JavaWizardPage::validatePage()
{
    ...
    if (result == JavaWizardWidget::ValidationStatus::AllOK) {
        conf.javaPath = m_java_widget->javaPath();
    }

    // Memory
    conf.minMemAlloc = m_java_widget->minHeapSize();
    conf.maxMemAlloc = m_java_widget->maxHeapSize();
    if (m_java_widget->permGenEnabled()) {
        conf.permGen = m_java_widget->permGenSize();
    } else {
